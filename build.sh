...
.analysis-table tr.drilldown-sub { display: none; }
.analysis-table tr.drilldown-sub.open { display: table-row; }
.analysis-table tr.drilldown-sub td { background: #f8faff; font-size: .78rem; color: var(--text-muted); border-bottom: 1px solid var(--border); }
.analysis-table tr.drilldown-sub td:first-child { padding-left: 34px; }
.analysis-table tr.drilldown-sub:hover td { background: #eef2f8; }
...
.modal-footer { padding: 10px 20px; background: var(--surface); border-radius: 0 0 12px 12px; font-size: .75rem; color: var(--text-muted); border-top: 1px solid var(--border); flex-shrink: 0; display: flex; align-items: center; justify-content: space-between; gap: 12px; }
.csv-btn { background: var(--primary); color: #fff; border: none; border-radius: 6px; padding: 5px 13px; font-size: .75rem; font-weight: 600; cursor: pointer; display: flex; align-items: center; gap: 5px; white-space: nowrap; transition: background .15s; flex-shrink: 0; }
.csv-btn:hover { background: #243d6a; }
...
function sortPairsByWL(pairs, pairsObj) {
  return pairs.slice().sort((a, b) => {
    const sa = pairsObj[a], sb = pairsObj[b];
    const ta = sa.wins + sa.losses, tb = sb.wins + sb.losses;
    if (tb !== ta) return tb - ta;
    if (sb.wins !== sa.wins) return sb.wins - sa.wins;
    const pctA = ta ? sa.wins / ta : 0, pctB = tb ? sb.wins / tb : 0;
    return pctB - pctA;
  });
}
...
function renderLineAnalysis(teamName, mbt) {
  ...
  lines.forEach(lineNum => {
    ...
    const grp = 'dd-' + (_ddUid++);
    rows += '<tr class="drilldown-header" data-grp="' + grp + '">' +
      '<td><strong>Line ' + lineNum + '</strong></td>' +
      '<td>' + wlHtml(ls.wins, ls.losses) + '</td>' +
      '</tr>';
    const pairs = sortPairsByWL(Object.keys(ls.pairs), ls.pairs);
    pairs.forEach(pk => {
      const ps = ls.pairs[pk];
      rows += '<tr class="drilldown-sub ' + grp + '">' +
        '<td>' + esc(pk) + '</td>' +
...
function renderPartnerAnalysis(teamName, mbt) {
  ...
  const pairs = sortPairsByWL(Object.keys(stats), stats);
...
document.addEventListener('click', function(e) {
  ...
  if (grp) {
    ...
    hdr.closest('tbody').querySelectorAll('tr.' + grp).forEach(function(r) {
...
function renderSeriesPane(data, paneEl, mbt, ptmMap) {
  ...
  data.forEach(div => {
    ...
    const divAvgPTINum = allPlayers.length > 0
      ? allPlayers.reduce((s,p) => s + p.rating, 0) / allPlayers.length : null;
    const divAvgPTI = divAvgPTINum !== null ? divAvgPTINum.toFixed(2) : '-';
    ...
    div.teams
      ...
      .forEach(team => {
        ...
        const belowAvg = (Array.isArray(team.players) && divAvgPTINum !== null)
          ? team.players.filter(p => p.rating < divAvgPTINum).length : '-';
        ...
        thead.innerHTML =
          ...
          '<div class="sw-team-stats">' +
            ...
            '<div class="sw-stat"><span class="sw-stat-val">' + belowAvg + '</span><span class="sw-stat-lbl" title="Number of rostered players with a PTI below the division average">Players Below Avg.</span></div>' +
...
function openModal(loc) {
  ...
  function downloadCSV() {
    ...
    const headers = ['First','Last','Team(s)','Division','Start PTI','Diff','Current PTI'];
    const rows = sorted.map(p => {
      ...
      return [
        ...
        (p.teamNames||[]).join('; '),
        p.division||'',
        p.startRating != null ? p.startRating : '',
        d.toFixed(1),
        p.currentRating != null ? p.currentRating : ''
    ...
    a.href = url;
    a.download = loc.name.replace(/[^a-z0-9]/gi,'_') + '_players.csv';
    a.click();
  ...
  document.getElementById('modalFooter').innerHTML =
    '<span>' + players.length + ' player' + (players.length!==1?'s':'') + ' — click any column header to sort</span>' +
    '<button class="csv-btn" id="csvDownloadBtn">&#8595; Download CSV</button>';
