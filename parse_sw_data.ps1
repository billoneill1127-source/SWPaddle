...
foreach ($token in $swTeamOrder) {
    ...
    try {
        ...
        # Parse player roster: name, rating, wins, losses
        $playerPat = '<a class="lightbox-auto iframe link"[^>]*/player\.php[^>]*>([^<]+)</a>[^<]*(?:\(C\))?\s*</div></td><td>\s*([\-\d.]+)\s*</td>\s*<td[^>]*><span class="md-font-gray">(\d+)</span></td>\s*<td[^>]*><span class="md-font-gray">(\d+)</span></td>'
        ...
        foreach ($pr in $playerRows) {
            $pName  = [System.Net.WebUtility]::HtmlDecode($pr.Groups[1].Value.Trim()) -replace '\s+', ' '
            $rating = 0.0; [double]::TryParse($pr.Groups[2].Value.Trim(),
                [System.Globalization.NumberStyles]::Any,
                [System.Globalization.CultureInfo]::InvariantCulture, [ref]$rating) | Out-Null
            $wins   = [int]$pr.Groups[3].Value
            $losses = [int]$pr.Groups[4].Value
